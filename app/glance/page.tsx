...
export default function GlancePage({embedded=false}:{embedded?:boolean}) {
  ...
  useEffect(()=>{fetch("/data/daily-brief/latest.json",{cache:"no-store"}).then(r=>r.ok?r.json():Promise.reject()).then(setBrief).catch(()=>setBrief(null))},[]);
  ...
  return <main className={`glance-page ${embedded?"glance-embedded":""}`}>
    ...
    <section className="glance-grid">
      ...
      <article className="g-card g-news daily-brief"><CardHead index="06" title="Alex 每日情报" meta="08:00 · DUBAI"/><div className="news-lead"><strong>{brief?.headline||"今天真正值得知道的事"}</strong><span>{brief?formatBriefDate(brief.date):"下一期正在准备"}<br/>{brief?`${brief.readingMinutes} 分钟阅读`:"每日 08:00 更新"}</span></div>{brief?.summary&&<p className="brief-summary">{brief.summary}</p>}<div className="brief-sections">{(brief?.sections||defaultBriefSections).map((section,index)=><section key={section.id}><header><span>{String(index+1).padStart(2,"0")}</span><h3>{section.title}</h3></header>{section.items.length?section.items.map((item,itemIndex)=><article className="brief-item" key={`${section.id}-${itemIndex}`}><div className="brief-item-head"><span>{item.source||"已核验来源"}</span>{item.url&&<a href={item.url} target="_blank" rel="noreferrer">原文 ↗</a>}</div><h4>{item.title}</h4>{item.happened&&<p>{item.happened}</p>}{item.why&&<p><b>为什么重要</b>{item.why}</p>}{item.relevance&&<p><b>与你的关系</b>{item.relevance}</p>}{item.opportunityRisk&&<p><b>机会与风险</b>{item.opportunityRisk}</p>}{item.action&&<p className="brief-action"><b>行动</b>{item.action}</p>}</article>):<p className="brief-empty">今日无必须关注更新</p>}</section>)}</div><small className="news-source-note">{brief?.sourceNote||"每日 08:00 自动同步 · 原始来源与链接保留 · 事实与判断分开呈现"}</small></article>
...
const defaultBriefSections:BriefSection[]=["今日必知","阿联酋本地","沙特与中东非","消费电子与电商","华为与竞品","AI 与全球科技","宏观经济与商业","中国观察","全球大事","今日行动提示"].map((title,index)=>({id:`section-${index+1}`,title,items:[]}));
